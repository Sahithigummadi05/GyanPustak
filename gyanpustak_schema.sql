-- ═══════════════════════════════════════════════════════════════════
-- GyanPustak Database Schema  —  NORMALIZED & CORRECTED
-- Changes from original:
--   1. phone removed from student  → student_phone table (1NF fix)
--   2. phone removed from employee → employee_phone table (1NF fix)
--   3. assigned_to_admin added to trouble_ticket ERD notes
--   4. All normalization-derived junction tables clearly marked
-- ═══════════════════════════════════════════════════════════════════
...
-- ─────────────────────────────────────────────────────────────────
-- EMPLOYEE  (base entity for Customer Support and Administrator)
-- phone removed → see employee_phone below (1NF: multivalued attr)
-- ─────────────────────────────────────────────────────────────────
...
-- ── 1NF FIX: phone is multivalued for employee ───────────────────
-- Added due to normalization (1NF — no repeating/multi-valued attr)
...
-- ─────────────────────────────────────────────────────────────────
-- ADMINISTRATOR  (specialization of EMPLOYEE)
-- is_super_admin acts as a discriminator; Super Admin is the role
-- that can add/remove other employees (shown in ER as ISA hierarchy)
-- ─────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS administrator (
    admin_id       INT AUTO_INCREMENT PRIMARY KEY,
    employee_id    INT NOT NULL UNIQUE,
    is_super_admin TINYINT(1) DEFAULT 0,
    added_by       INT,                          -- FK to administrator (super admin adds)
    FOREIGN KEY (employee_id) REFERENCES employee(employee_id),
    FOREIGN KEY (added_by)    REFERENCES administrator(admin_id)
);

-- ─────────────────────────────────────────────────────────────────
-- CUSTOMER SUPPORT  (specialization of EMPLOYEE)
-- ─────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS customer_support (
    cs_id       INT AUTO_INCREMENT PRIMARY KEY,
    employee_id INT NOT NULL UNIQUE,
    added_by    INT,                             -- FK to administrator
    FOREIGN KEY (employee_id) REFERENCES employee(employee_id),
    FOREIGN KEY (added_by)    REFERENCES administrator(admin_id)
);
...
CREATE TABLE IF NOT EXISTS university (
    ...
    rep_phone     VARCHAR(15)       -- university rep has one phone; kept atomic
);
...
-- ── NORMALIZATION TABLE: M:N Course ↔ Department ─────────────────
-- Added due to normalization (2NF: removes partial/multi-dependency)
...
-- ── NORMALIZATION TABLE: M:N Course ↔ Instructor ─────────────────
-- Added due to normalization (M:N decomposition)
...
-- ─────────────────────────────────────────────────────────────────
-- STUDENT
-- phone removed → see student_phone below (1NF fix)
-- ─────────────────────────────────────────────────────────────────
...
-- ── 1NF FIX: phone is multivalued for student ────────────────────
-- Added due to normalization (1NF — no repeating/multi-valued attr)
...
-- subcategory separated from category to satisfy 3NF
-- (name depends only on subcategory_id, category_id is a FK — not transitive)
...
-- ── KEYWORD entity (was missing from ER diagram) ─────────────────
...
-- ── NORMALIZATION TABLE: M:N Book ↔ Author ───────────────────────
-- Added due to normalization (M:N decomposition)
...
-- ── NORMALIZATION TABLE: M:N Book ↔ Keyword ──────────────────────
-- Added due to normalization (1NF: keywords can't be stored as a
-- comma-separated list inside book; M:N decomposition)
-- NOTE: This entity + relationship was MISSING from your ER diagram
...
-- ── NORMALIZATION TABLE: M:N Book ↔ Subcategory ──────────────────
-- Added due to normalization (M:N decomposition)
...
-- ── COURSE_BOOK: M:N Book ↔ Course (with instructor attribution) ─
CREATE TABLE IF NOT EXISTS course_book (
    id               INT AUTO_INCREMENT PRIMARY KEY,
    course_id        INT NOT NULL,
    book_id          INT NOT NULL,
    instructor_id    INT,                         -- which instructor assigned this book
...
CREATE TABLE IF NOT EXISTS cart (
    ...
    student_id   INT NOT NULL UNIQUE,            -- 1 cart per student (UNIQUE enforces it)
...
-- ─────────────────────────────────────────────────────────────────
-- TROUBLE TICKET
-- assigned_to_admin added (was in SQL but missing from ER diagram)
-- ─────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS trouble_ticket (
    ...
    assigned_to_admin  INT,                      -- also add this to your ER diagram!
...
-- ── NORMALIZATION TABLE: TICKET_STATUS_HISTORY ───────────────────
-- Added due to normalization (1NF: status change history is a
-- repeating group — must be extracted into its own table)
-- NOTE: This entity was MISSING from your ER diagram
...
('Electronics', 2),      ('Computer Science', 2);
...
-- ── Employees ────────────────────────────────────────────────────
-- Super Admin
INSERT INTO employee (fname, lname, gender, salary, aadhaar, email, address, password) VALUES
('Super', 'Admin', 'Male', 150000.00, '111122223333', 'superadmin@gyanpustak.com', 'HQ, New Delhi', 'superadmin123');
INSERT INTO administrator (employee_id, is_super_admin, added_by) VALUES (1, 1, NULL);
INSERT INTO employee_phone (employee_id, phone) VALUES (1, '9000000001');

-- Regular Admin
INSERT INTO employee (fname, lname, gender, salary, aadhaar, email, address, password) VALUES
('Arun', 'Verma', 'Male', 80000.00, '444455556666', 'arun.admin@gyanpustak.com', 'Connaught Place, Delhi', 'admin123');
INSERT INTO administrator (employee_id, is_super_admin, added_by) VALUES (2, 0, 1);
INSERT INTO employee_phone (employee_id, phone) VALUES (2, '9000000002');

-- Customer Support
INSERT INTO employee (fname, lname, gender, salary, aadhaar, email, address, password) VALUES
('Meena', 'Raj', 'Female', 50000.00, '777788889999', 'meena.cs@gyanpustak.com', 'Lajpat Nagar, Delhi', 'cs123');
INSERT INTO customer_support (employee_id, added_by) VALUES (3, 1);
INSERT INTO employee_phone (employee_id, phone) VALUES (3, '9000000003');

-- ── Students ─────────────────────────────────────────────────────
INSERT INTO student (fname, lname, email, address, dob, major, status, year_of_study, university_id, password) VALUES
('Rohit',  'Singh', 'rohit@student.com',  'Delhi',  '2002-05-10', 'Computer Science', 'Undergraduate', 3, 1, 'student123'),
('Anjali', 'Mehta', 'anjali@student.com', 'Mumbai', '2001-08-22', 'Mathematics',      'Undergraduate', 4, 1, 'student456');

-- Multivalued phones for students
INSERT INTO student_phone (student_id, phone) VALUES
(1, '9111111111'),
(2, '9222222222'),
(2, '9333333333');  -- Anjali has two phones — this is why multivalued is needed
...
('2025-01-10', 'Book not available', 'Introduction to Algorithms shows out of stock but website still shows available.', 'New',      'Products', 1),
...
-- Ticket status history (was missing from original sample data)
INSERT INTO ticket_status_history (ticket_id, old_status, new_status, changed_by_employee) VALUES
(2, 'New', 'Assigned', 3);   -- CS employee Meena assigned ticket 2
